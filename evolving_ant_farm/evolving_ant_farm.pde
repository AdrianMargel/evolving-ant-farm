...
class Ant {
  ...
  Ant(Ant parAnt, Vector p) {
    ...
    hue=(parAnt.hue+(int)random(-hueChange, hueChange))%256;
    if (hue<0) {
      hue+=256;
    }
  ...
  void addRules() {
    ...
    for (int i=0; i<add; i++) {
      ...
      if(eightDirections){
        tempRule=new Rule(temp, (int)random(0, tileTypes), (int)random(0, 8));
      }else{
